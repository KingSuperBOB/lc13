...
/obj/item/lobotomizer/attack_self(mob/living/carbon/human/user)
	if(!(user.has_quirk(/datum/quirk/brainproblems)) || !(istype(user)) || lobotomizing)
		...
		return
	user.add_overlay(mutable_appearance('ModularLobotomy/_Lobotomyicons/tegu_effects.dmi', "lobotomizer", -HALO_LAYER))
...
/obj/item/lobotomizer/proc/EndLoop(mob/living/user)
	user.cut_overlay(mutable_appearance('ModularLobotomy/_Lobotomyicons/tegu_effects.dmi', "lobotomizer", -HALO_LAYER))
